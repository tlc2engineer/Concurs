...
(select id,similarity(20126,id) as s 
from (select distinct id from likes where pid in 
(select pid from likes where id=20126) and not id=20126) as t
---where id  in (select id from accounts where city='Варанск')
order by s desc) as t1
on likes.id=t1.id
where pid not in (select pid from likes where id=20126))as t3
group by pid 
)as t4
